...
const CATEGORY_TABS = ['All', 'Food', 'Transport', 'Bills', 'Shopping']

// ─── Color Tokens (matches Group_expense.tsx) ───────────────────────────────
const C = {
  navy:          '#0F1923',
  navyLight:     '#1A2B3C',
  amber:         '#EAAF35',
  amberMuted:    'rgba(234,175,53,0.15)',
  amberBorder:   'rgba(234,175,53,0.3)',
  white:         '#FFFFFF',
  surface:       '#F4F6F9',
  card:          '#FFFFFF',
  textPrimary:   '#0F1923',
...
export default function HomeScreen() {
  ...
  useEffect(() => {
    if (!user) return
    const unsub = subscribeToUserExpenses(user.uid, (fetched) => {
      dispatch(setExpenses(fetched))
    })
    return () => unsub()
  }, [user, dispatch])
  ...
  useEffect(() => {
    if (!user) return
    const unsub = subscribeToUserGroups(user.uid, (fetchedGroups) => {
      dispatch(setGroups(fetchedGroups))
      const unsubs: (() => void)[] = []
      fetchedGroups.forEach(group => {
        const u = subscribeToGroupExpenses(group.id, (expenses) => {
  ...
  useEffect(() => {
    const combined: CombinedExpense[] = [
      ...personalExpenses.map(exp => ({
        ...
        category: exp.category, date: exp.date, description: exp.description,
  ...
  const filteredExpenses = allExpenses
    .slice(0, 6)
    .filter(e => activeTab === 'All' || e.category.toLowerCase() === activeTab.toLowerCase())
  ...
  const getBudgetColor = () => {
    if (percentage >= 100) return '#E85B4A'
  ...
  const deleteGroupCompletely = (groupId: string, groupName: string) => {
    Alert.alert('Delete Group', `Delete "${groupName}" and all its expenses?`, [
  ...
  const getCategoryBg = (cat: string) => ({
    Food: '#FFF3E0', Transport: '#E8F4FD', Shopping: '#FDE8E8',
    Bills: '#E0F7FA', Health: '#FCE4EC', Other: '#F3F4F6',
  }[cat] || '#F3F4F6')
  ...
  const renderExpenseItem = ({ item }: { item: CombinedExpense }) => (
    <View style={styles.expenseItem}>
      <View style={[styles.expIcon, { backgroundColor: getCategoryBg(item.category) }]}>
    ...
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor: C.navy }}>
        <View style={styles.loginPrompt}>
          ...
          <Text style={styles.loginSub}>Sign in to view your expenses and groups</Text>
          ...
          <>
            ...
            >
              ...
              <View style={styles.spendCard}>
                ...
                <Text style={styles.spendAmount}>₹{total.toLocaleString()}</Text>
                {monthlyLimit > 0 && (
                  <View>
                    ...
                    <View style={styles.budgetFooter}>
                      <Text style={styles.budgetPct}>{percentage.toFixed(0)}% used</Text>
            ...
            <View style={styles.quickRow}>
              ...
              <TouchableOpacity
                ...
                onPress={() => router.push('/Group_creation')}
              >
                <Text style={styles.quickIcon}>👥</Text>
                <Text style={styles.quickLabel}>New Group</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={styles.quickBtn}
              ...
              >
                <Text style={styles.quickIcon}>🔗</Text>
                <Text style={styles.quickLabel}>Join Group</Text>
              </TouchableOpacity>
            </View>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                {groups.length === 0 ? (
                  <View style={styles.emptyGroupCard}>
                    <Text style={styles.emptyGroupText}>No groups yet. Create or join one!</Text>
                  </View>
                ) : (
                  groups.map(group => (
                    <TouchableOpacity
                    ...
                    >
                      ...
                      <View style={styles.groupBarTrack}>
                        <View style={[styles.groupBarFill, {
                          width: `${Math.min((group.totalExpenses / 10000) * 100, 100)}%` as any,
                        }]} />
                      </View>
                    </TouchableOpacity>
                  ))
            ...
            <View style={styles.section}>
              <View style={styles.sectionRow}>
                ...
                <TouchableOpacity onPress={() => router.push('/all-expenses' as any)}>
                  <Text style={styles.sectionLinkMuted}>See all</Text>
                </TouchableOpacity>
              </View>
...
const styles = StyleSheet.create({
  ...
  listContent: { paddingBottom: 100, backgroundColor: C.surface },
  ...
  avatarBtn: {
    ...
    height: 46,
    borderRadius: 15,
    backgroundColor: 'rgba(234,175,53,0.15)',
    borderWidth: 1.5,
    borderColor: 'rgba(234,175,53,0.3)',
    alignItems: 'center',
  ...
  spendLabel: { fontSize: 12, color: 'rgba(255,255,255,0.5)', fontWeight: '500', marginBottom: 6 },
  spendAmount: {
    fontSize: 40,
    fontWeight: '800',
    color: C.white,
  ...
  budgetPct: { fontSize: 11, color: 'rgba(234,175,53,0.8)', fontWeight: '700' },
  budgetRemaining: { fontSize: 11, color: 'rgba(255,255,255,0.4)', fontWeight: '500' },
  ...
  quickLabel: { fontSize: 11, color: C.amber, fontWeight: '700', letterSpacing: 0.2 },

  // ── Sections ──
  section: { paddingHorizontal: 16, paddingTop: 20, backgroundColor: C.surface },
  sectionRow: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
  ...
  sectionLinkMuted: { fontSize: 13, color: C.textSecondary, fontWeight: '600' },
  sectionLinkBtn: {
    backgroundColor: C.navy,
  ...
  sectionLinkBtnText: { fontSize: 13, color: C.amber, fontWeight: '700' },
  ...
  groupBadgeActive: { backgroundColor: 'rgba(234,175,53,0.15)' },
  groupBadgeEmpty: { backgroundColor: '#F1F5F9' },
  groupBadgeText: { fontSize: 10, fontWeight: '700' },
  groupMembers: { fontSize: 11, color: C.textSecondary, marginBottom: 10 },
  groupBarTrack: {
    height: 3,
  ...
  groupBarFill: { height: 3, borderRadius: 2, backgroundColor: C.amber },
  emptyGroupCard: {
    width: 220,
    backgroundColor: C.card,
    borderRadius: 20,
  ...
  emptyGroupText: { fontSize: 13, color: C.textSecondary, textAlign: 'center' },
  ...
  chipTextActive: { color: C.amber, fontWeight: '700' },
  ...
  expSub: { fontSize: 12, color: C.textSecondary },
  groupTag: { color: C.green, fontWeight: '600' },
  expAmount: { fontSize: 15, fontWeight: '700', color: C.red, letterSpacing: -0.3 },
  ...
  fabText: { fontSize: 30, color: C.navy, fontWeight: '300', marginTop: -2 },

  // ── Bottom Sheet ──
  sheetBg: { backgroundColor: C.white, borderRadius: 24 },
  sheetHandle: { backgroundColor: C.border, width: 40 },
  sheetContent: { flex: 1, paddingHorizontal: 20, paddingBottom: 30 },
  profileAvatarWrap: { alignItems: 'center', marginTop: 20, marginBottom: 14 },
  profileAvatar: {
    width: 76,
    height: 76,
    borderRadius: 24,
  ...
  profileAvatarText: { fontSize: 28, fontWeight: '800', color: C.amber },
  profileName: {
    textAlign: 'center',
    fontSize: 20,
    fontWeight: '700',
  ...
  profileEmail: { textAlign: 'center', fontSize: 13, color: C.textSecondary, marginBottom: 20 },
  profileDivider: { height: 1, backgroundColor: C.border, marginBottom: 12 },
  menuItem: { flexDirection: 'row', alignItems: 'center', paddingVertical: 14, gap: 14 },
  menuIcon: { fontSize: 22, width: 30 },
  menuLabel: { fontSize: 16, color: C.textPrimary, fontWeight: '500' },
  logoutBtn: {
    flexDirection: 'row',
    alignItems: 'center',
    marginTop: 12,
  ...
  versionText: { textAlign: 'center', fontSize: 12, color: C.textSecondary, marginTop: 20 },
  ...
  loginEmoji: { fontSize: 72, marginBottom: 24 },
  loginTitle: { fontSize: 24, fontWeight: '700', color: C.white, marginBottom: 12 },
  loginSub: { fontSize: 14, color: 'rgba(255,255,255,0.55)', textAlign: 'center', marginBottom: 32 },
  loginBtn: {
    backgroundColor: C.amber,
    paddingHorizontal: 32,
    paddingVertical: 14,
  ...
  loginBtnText: { color: C.navy, fontSize: 16, fontWeight: '700' },
})
...
//   // Total Card
//   totalCard: { backgroundColor: 'rgba(48, 35, 35, 0.15)', borderRadius: 20, padding: 16 },
